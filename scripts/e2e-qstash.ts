...
} catch { /* .env.local missing — use existing env */ }

const requiredEnv = ['QSTASH_TOKEN', 'QSTASH_CURRENT_SIGNING_KEY', 'QSTASH_NEXT_SIGNING_KEY', 'GOOGLE_API_KEY', 'TURSO_DATABASE_URL', 'TURSO_AUTH_TOKEN'];
...
async function run() {
  ...
  try {
    ...
    if (hasSourceId && hasCreatedAt) {
      ...
      console.error(`❌ Schema check failed. Missing: ${!hasSourceId ? 'source_id ' : ''}${!hasCreatedAt ? 'created_at' : ''}`);
      exit(1);
    }
  } catch (e) {
  ...
  let initialCount = 0;
  try {
    const before = await db.execute("SELECT COUNT(*) as cnt FROM articles WHERE score IS NOT NULL AND score > 0");
  ...
  while (Date.now() - startTime < timeout) {
    try {
      const result = await db.execute("SELECT COUNT(*) as cnt FROM articles WHERE score IS NOT NULL AND score > 0");
