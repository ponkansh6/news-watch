...
| **Tier 4: Data Access**            | `db/{repository,query}/*.ts` (`actions.ts` is re-export barrel, R3-a planned)                                                 | DB CRUD操作。Drizzle ORMラッパー                     | **>65%**                        | 全CRUD操作の正常系をカバー。catch節のエラーハンドリングは軽量で可。**Note**: `actions.ts` は将来 Phase で `lib/db/index.ts` へ統合予定       |
